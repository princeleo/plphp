...
/**
 * Class System_Lib_DbDriver
 * @author leo.zou
 * @param $params
 * @param $active_record_override
 */
class System_Lib_DbDriver
 {
    var $params;
    ...
    public function __construct($params = '', $active_record_override = NULL)
    {
        ...
        if ($active_record_override !== NULL){
            $this->active_record = $active_record_override;
        }
    ...
    {
        $DB = new System_Lib_DbDriver($params, $active_record_override);
        if($DB->autoinit == TRUE){
            $DB->initialize();
        }
    ...
    protected function db_set_charset($charset, $collation)
    {
        if ( ! $this->_db_set_charset($this->char_set, $this->dbcollat)){
            $this->log_message('Unable to set database connection charset: '.$this->char_set);
            return FALSE;
        }
    ...
    function query($sql, $binds = FALSE, $return_object = TRUE)
    {
        if ($sql == '')
        {
            if ($this->db_debug)
            {
                $this->log_message( 'Invalid query: '.$sql);
        ...
        if (FALSE === ($this->result_id = $this->simple_query($sql)))
        {
            ...
            if ($this->db_debug)
            {
                ...
                $this->log_message( 'Query error: '.$error_msg);
    ...
    function trans_complete()
    {
        ...
        if ($this->_trans_status === FALSE)
        {
            ...
            $this->log_message('DB Transaction Failure');
            return FALSE;
        }
     ...
     * @param string $log
     */
    protected function log_message($log =  '')
    {
        if(empty($log)) return ;

        @file_put_contents(DEBUG_LOG_PATH.'DB_'.date('Y-m-d').'.log', $log."\n", FILE_APPEND);
    }
 }